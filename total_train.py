...
for epoch in range(last_epoch, total_epoches):
    ...
    if metric.image_auc > max_auc:
        ...
        torch.save(model.state_dict(),
                   os.path.join(args.save_dir,f"{args.split_name}_all_{args.configs['save_name']}"))
        print("Model saved as {}".format(os.path.join(args.save_dir,f"{args.split_name}_all_{args.configs['save_name']}")))
    
    metric.reset()
model.load_state_dict(
    torch.load(os.path.join(args.save_dir,f"{args.split_name}_all_{args.configs['save_name']}"))
)
