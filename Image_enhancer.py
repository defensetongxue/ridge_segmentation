...
if __name__=='__main__':
    ...
    image_list=os.listdir(os.path.join(args.data_path,'images'))
    os.makedirs(os.path.join(args.data_path,'enhanced'),exist_ok=True)
    cnt=0
    for image_name in image_list:
        image_path = os.path.join(args.data_path,'images',image_name)
        image = Image.open(image_path)
        image_tensor = transforms.ToTensor()(image)
        enhanced_tensor = enhance_transform(image_tensor)
        enhanced_path=os.path.join(args.data_path,'enhanced',image_name)
        ...
        cnt+=1
    print("finish enhance image number: ",str(cnt))