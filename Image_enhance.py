...
if __name__=='__main__':
    ...
    os.makedirs(os.path.join(data_path,'enhanced_image'),exist_ok=True)
    for image_name in image_list:
        image_path = os.path.join(data_path,'images',image_name)
        image = Image.open(image_path)
        image_tensor = transforms.ToTensor()(image)
        enhanced_tensor = enhance_transform(image_tensor)
        enhanced_path=os.path.join(data_path,'enhanced_image',image_name)
